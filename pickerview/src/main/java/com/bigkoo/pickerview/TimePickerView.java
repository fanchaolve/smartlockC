...
public class TimePickerView extends BasePickerView implements View.OnClickListener {
    ...
    private void initView(Context context) {
        ...
        if (customListener == null) {
            ...
            mLayoutView = LayoutInflater.from(context).inflate(layoutRes, contentContainer);
            mLayoutView.setOnClickListener(null);
            customListener.customLayout(mLayoutView);
        }
        // 时间转轮 自定义控件
        mTimePickerView = (LinearLayout) findViewById(R.id.timepicker);
        mTimePickerView.setVisibility(visibility);
        mTimePickerView.setBackgroundColor(Color_Background_Wheel == 0 ? bgColor_default : Color_Background_Wheel);

        wheelTime = new WheelTime(mTimePickerView, type, gravity, Size_Content);
        wheelTime.setLunarCalendar(isLunarCalendar);
        wheelTime.setOnSelectedListener(new WheelTime.OnSelectedListener() {
            @Override
    ...
    public void returnData() {
        if (timeSelectListener != null) {
            try {
                ...
                timeSelectListener.onTimeSelect(date, mLayoutView);
